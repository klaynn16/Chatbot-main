...
function renderDashboard(el) {
  fetch("http://localhost/admin_db/api/dashboard.php")
    .then(res => res.json())
    .then(data => {
      const totalChats = data.totalChats || 0;
      const kbEntries = data.kbEntries || 0;
      const annCount = data.announcements || 0;
    ...
    .catch(err => {
      ...
      el.innerHTML = `<div class="p-6 text-red-500">Failed to load dashboard data.</div>`;
    });
}
...
function renderChatLogs(el) {
  fetch("http://localhost/admin_db/api/chatlogs/get.php")
    .then(res => res.json())
    .then(logs => {
                      ...
                      ${sortedLogs.map((log, index) => {
                        ...
                        return `
                          <tr class="hover:bg-muted/30 transition-colors">
                            <td class="px-4 py-3 font-medium whitespace-nowrap">${log.student_number}</td>
                ...
                ${sortedLogs.map((log, index) => {
                  ...
                  return `
                    <div class="bg-card rounded-xl border border-border p-4">
                      ...
                      <p class="text-sm font-medium mb-2">${log.student_number}</p>
      ...
      document.getElementById('clear-chatlogs-btn')?.addEventListener('click', () => {
        fetch("http://localhost/admin_db/api/chatlogs/delete.php")
          .then(res => res.json())
          .then(() => {
            renderChatLogs(el);
          });
      ...
      document.querySelectorAll('.view-details-btn').forEach(btn => {
        btn.addEventListener('click', () => {
          ...
          document.getElementById('modal-studentNumber').textContent = log.student_number;
    ...
    .catch(err => {
      ...
      el.innerHTML = `<div class="p-6 text-red-500">Failed to load chat logs.</div>`;
    });
};
